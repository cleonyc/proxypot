...
impl Database {
    ...
    pub async fn handle_login(
        ...
        ip: &str,
        mut username: String,
    ) -> anyhow::Result<(Client, Login)> {
        let mut resp = isahc::get_async(format!(
...
async fn get_ipinfo(ip: &str) -> anyhow::Result<String> {
    let request = Request::get(format!("https://ipinfo.io/widget/demo/{}", ip))
    ...
    Ok(format!(
        ...
        json["data"]["company"]["name"]
            .as_str()
            .unwrap_or("Unknown"),
        json["data"]["region"].as_str().unwrap_or("Unknown"),
        json["data"]["country"].as_str().unwrap_or("Unknown")
    ))
}
...
async fn test_get_ipinfo() {
    ...
    assert_eq!(
        ipinfo,
        "Company: M247 LTD New York Infrastructure, Location: New York, US".to_string()
    )
}
